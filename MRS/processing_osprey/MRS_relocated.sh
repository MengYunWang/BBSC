...
for sub in `cat ./subjList.txt`; do 
    ...
    for ses_name in `cat ./sesList.txt`; do
        ...
        # cp $filepath/$sub/$ses_name/00*_PROBE-SV_35/P*.7 $filepath/$sub/$ses_name/00*_PROBE-SV_35/${sub}_ses-${ses_num}_P35.7
        # mv $filepath/$sub/$ses_name/00*_PROBE-SV_35/${sub}_ses-${ses_num}_P35.7 $MRS_path/$sub/Pfile 
