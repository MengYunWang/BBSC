...
# Subset the sub3 78:102 rows and the specified columns
cols <- c("NAA_NAAG", "NAA", "Cr_PCr", "PCr", "Cr", "Glu_Gln", "Glu", "mI", "PCh_GPC")
subset1 <- file1[78:102, cols]
subset2 <- file2[78:102, cols]
subset3 <- file3[78:102, cols]
subset4 <- file4[78:102, cols]
...
CV_sub3 <- data.frame(column = c("NAA_NAAG", "NAA", "Cr_PCr", "PCr", "Cr", "Glu_Gln", "Glu", "mI", "PCh_GPC"), tCr = cv1, water_raw = cv2, water_CSF = cv3, water_tissue_corrected = cv4)
...
write.table(CV_sub3, file = paste(output_dir, "sub1_CV.tsv", sep = "/"), sep = "\t", row.names = FALSE)
