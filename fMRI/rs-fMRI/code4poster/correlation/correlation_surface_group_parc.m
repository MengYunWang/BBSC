%% compute the network correlation based on public yeo-7 parcellation
...
yeo7_parc = load ([parc_path '1000subjects_clusters007_ref.mat']);
...
for subi = 1:2
    ...
    for sesi = 1:length(ses_surf)
        ...
        data_network (:,:,sesi,subi) = extract_data_yeo7(lh_surf_data, rh_surf_data, yeo7_parc); % network * datapoints * sessions * sub
...
% save the data
save (fullfile(datasave_path, 'sub1_correlation_yeo.mat'), 'inter_session_corr_sub1', 'session_avg_corr_sub1', '-mat');
save (fullfile(datasave_path, 'sub2_correlation_yeo.mat'), 'inter_session_corr_sub2', 'session_avg_corr_sub2', '-mat');
cd (datasave_path)
...
for neti = 1:7
    lh_network_indx = (parc.lh_labels == neti);
    lh_network_data = lh_surf_data(lh_network_indx,:);

    rh_network_indx = (parc.rh_labels == neti);
