...
for subi,sub_name in enumerate(sub_list):
    func_files = layout.get(subject=sub_name,
    ...
    mask_files = layout.get(subject=sub_name,
                        datatype='func', task='rest',
                        desc='brain',
                        suffix='mask',
                        space='MNI152NLin6Asym',
    ...
    for sesi,ses_file in enumerate(func_files):
        ...
        clean_img.to_filename(data_save_dir + 'sub-%s/sub-%s_ses-%s_task-rest_space-MNI152NLin6Asym_res-2_desc-preproc_bold_clean_normalized.nii.gz'\
             % (str(subi+1).zfill(2),str(subi+1).zfill(2),str(sesi+1).zfill(2)))
