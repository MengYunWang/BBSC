...
percentage_change_sub1 <- calculate_percentage_change(data_sets$sub1, 1:34, 35:49)
percentage_change_sub2 <- calculate_percentage_change(data_sets$sub2, 1:34, 35:49)
percentage_change_sub3 <- calculate_percentage_change(data_sets$sub3, 1:34, 35:49)
...
sub_percent_long <- lapply(names(data_sets), function(name) {
  ...
  percentage <- calculate_percentage_change(df, 1:34, 35:49)
...
percentage_change_long <- rbind(sub_percent_long$sub1_long, sub_percent_long$sub2_long, sub_percent_long$sub3_long)
...
# with each subj
modell_day <- lapply(sub_percent_long, function(df) {
  normalized_df <- df %>%
    mutate_at(
      vars(
        cjv,
...
# within each subj
modell_time <- lapply(sub_percent_long, function(df) {
  normalized_df <- df %>%
    mutate_at(
      vars(
        day_length,
