...
data_sets <- lapply(file_paths, function(path) {
  ...
  data$total <- rowSums(data, na.rm = TRUE) # compute the total surface
  return(data)
})
names(data_sets) <-
  c("sub1_lh",
...
##---------------------------------------------Part II Plot the area from smallest to largest
sub1_long <-
  pivot_longer(
...
plots_area_ordered_qc <- plots_area_ordered
save(plots_area_ordered_qc, file = "plots_area_ordered_qc.RData")

percentage_change_qc <- percentage_change
write.csv(percentage_change_qc, "percentage_change_area_qc.csv", row.names = TRUE)

percentage_change_long_qc <- percentage_change_long
write.csv(percentage_change_long_qc, "percentage_change_area_long_qc.csv", row.names = TRUE)
...
model_day_area <- lapply(data_sets_long, function(df) {

  df <- filter(df, region == "total") # only look at the total area

  # Standardize the variables
  df$area_normalized <- scale(df$area)

  model <- lm(area_normalized ~ day, data = df)
...
data2meta_day <- data.frame(
  beta = sapply(model_day_area, function(x) x$beta["daymorning"]),
  se_beta = sapply(model_day_area, function(x) x$SE["daymorning"])
)

meta_model_day_area <- rma(yi=beta, sei=se_beta, data = data2meta_day)
summary(meta_model_day_area)
save(meta_model_day_area, file="meta_model_day_area.RData")

# 3. do the equivalence testing based on the meta results
library(TOSTER)
TOSTmeta(ES = summary(meta_model_day_area)$beta, se = summary(meta_model_day_area)$se,
...
model_time_area <- lapply(data_sets_long, function(df) {
  ...
  model <- lm(area_normalized ~ day_length, data = df)
...
data2meta_time <- data.frame(
  beta = sapply(model_time_area, function(x) x$beta["day_length"]),
  se_beta = sapply(model_time_area, function(x) x$SE["day_length"])
)

meta_model_time_area <- rma(yi=beta, sei=se_beta, data = data2meta_time)
summary(meta_model_time_area)
save(meta_model_time_area, file="meta_model_time_area.RData")

# 3. equivalence testing based on the meta results
TOSTmeta(ES = summary(meta_model_time_area)$beta, se = summary(meta_model_time_area)$se,
         low_eqbound_d=-0.5, high_eqbound_d=0.5, alpha=0.05)
