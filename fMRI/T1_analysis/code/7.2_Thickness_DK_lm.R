...
# 3. calculate the percentage change of the thickness
# 4. linear regression to time effects

# WANG. updated 09-Aug-2024
# WANG. 04-March-2024
...
data_sets <- lapply(file_paths, function(path) {
  ...
  data$average <- rowMeans(data, na.rm = TRUE) # compute the average thickness
  return(data)
})
...
data_sets[["sub3_lh"]][c(1,7), ] <- NA # delete the 1st and 7th, pay attention to the files names that need to be saved
...
# add the morning and afternoon indices
morning_sub1 <-
...
# add the daylight length (in minutes) at the data collection
...
# function to insert several columns: day, hemi, and daylight length
...
## -------------------------------------------- Part II. Plot the thickness from thinnest to thickest
...
process_data_and_plot <- function(data) {
  # calcualte the average thickness of each region
  average_thickness_region <- data %>%
    group_by(region) %>%
    summarise(avg_thickness = mean(thickness, na.rm = TRUE)) %>%
    arrange(avg_thickness) %>%
    mutate(region = factor(region, levels = region))

  # update the region
  data_updated <- data %>%
    filter(region != "average") %>%
    mutate(region = factor(region, levels = average_thickness_region$region))

  #
  average_thickness <- data_updated %>%
    group_by(region, hemi) %>%
    summarise(avg_thickness = mean(thickness, na.rm = TRUE)) %>%
    ungroup() %>%
    arrange(region, hemi)

  unique_regions <- unique(average_thickness$region)
  desired_breaks <-
    c(head(unique_regions, 3), tail(unique_regions, 3))

  #
  plot <-
    ggplot(data_updated, aes(x = region, y = thickness, color = hemi)) +
    geom_point(alpha = 0.3) +
    geom_line(
      data = average_thickness,
      aes(x = region, y = avg_thickness, group = hemi),
      size = 1
    ) +
    geom_point(
      data = average_thickness,
...
plots_thickness_ordered <- lapply(data_sets_long, process_data_and_plot)


### ------------------------------------------Part III calculate the percentage change of thickness
# 1. define function and compute the percentage change
...
columns_to_change <- 1:35 # brain regions to be computed
columns_to_keep <- 36:39 # other variable to be intact
...
percentage_change <- rbind(percentage_change_lh, percentage_change_rh)



# 2. transform to long form
percentage_change_long <- pivot_longer(
  ...
  names_to = "region",
  values_to = "thickness_per_change"
) %>%
  mutate(
    day = as.factor(day),
    hemi = as.factor(hemi),
...
# save(plots_thickness_ordered, file = "plots_thickness_ordered.RData")
# write.csv(percentage_change, "percentage_change_thickness.csv", row.names = TRUE)
# write.csv(percentage_change_long, "percentage_change_thickness_long.csv", row.names = TRUE)

# if session 1 and 7 were excluded
plots_thickness_ordered_qc <- plots_thickness_ordered
save(plots_thickness_ordered_qc, file = "plots_thickness_ordered_qc.RData")

percentage_change_qc <- percentage_change
write.csv(percentage_change_qc, "percentage_change_thickness_qc.csv", row.names = TRUE)

percentage_change_long_qc <- percentage_change_long
write.csv(percentage_change_long_qc, "percentage_change_thickness_long_qc.csv", row.names = TRUE)

# 3. list the data into its own
sub_per_change_long <-
  list(
...
model_day_thickness <- lapply(data_sets_long, function(df) {

  df <- filter(df, region == "average") # only look at the average thickness

  # Standardize the variables
  df$thickness_normalized <- scale(df$thickness)

  model <- lm(thickness_normalized ~ day, data = df)
...
data2meta_day <- data.frame(
  beta = sapply(model_day_thickness, function(x) x$beta["daymorning"]),
  se_beta = sapply(model_day_thickness, function(x) x$SE["daymorning"])
)

meta_model_day_thickness <- rma(yi=beta, sei=se_beta, data = data2meta_day)
summary(meta_model_day_thickness)
save(meta_model_day_thickness, file="meta_model_day_thickness.RData")

# 3. do the equivalence testing based on the meta results
library(TOSTER)
TOSTmeta(ES = summary(meta_model_day_thickness)$beta, se = summary(meta_model_day_thickness)$se,
...
model_time_thickness <- lapply(data_sets_long, function(df) {

  df <- filter(df, region == "average")

  # Standardize the variables
  df$thickness_normalized <- scale(df$thickness)
  df$day_length <- scale(df$day_length)

  model <- lm(thickness_normalized ~ day_length, data = df)
...
data2meta_time <- data.frame(
  beta = sapply(model_time_thickness, function(x) x$beta["day_length"]),
  se_beta = sapply(model_time_thickness, function(x) x$SE["day_length"])
)

meta_model_time_thickness <- rma(yi=beta, sei=se_beta, data = data2meta_time)
summary(meta_model_time_thickness)
save(meta_model_time_thickness, file="meta_model_time_thickness.RData")

# 3. equivalence testing based on the meta results
TOSTmeta(ES = summary(meta_model_time_thickness)$beta, se = summary(meta_model_time_thickness)$se,
         low_eqbound_d=-0.5, high_eqbound_d=0.5, alpha=0.05)
...
#   ) %>%
#   lmer(
#     thickness_per_change ~ cjv + efc + fwhm_avg + wm2max + fber + inu_med +
...
#     )
#   lmer(
#     thickness_per_change ~ day * region + cjv + efc + fwhm_avg + wm2max + 
...
#   lmer(
#     thickness_per_change ~ day * region + cjv + efc + fwhm_avg + wm2max +
#       fber_log + inu_med + cnr + snr_total + rpve_gm + (1 | subject),
#     data = .
#   )
# 
...
#   ) %>%
#   lmer(
#     # thickness_per_change ~ day_length * region + cjv + efc + fwhm_avg + wm2max +
#     #   fber + inu_med + cnr + snr_total + rpve_gm + (1 | subject),
#     thickness_per_change ~ day_length * region + (1 | subject),
#     data = .
#   )
# 
...
# 
# p_adjusted <- p.adjust(p_values, method = "BH")
# p_adjusted_small <- p_adjusted[p_adjusted < 0.05] %>%
#   print()
