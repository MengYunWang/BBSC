...
# function to plot the data with ggseg
plot_cv_area <- function(cv_area_data) {
  ggplot(cv_area_data) +
    geom_brain(atlas = dk, 
...
plot_mean_area <- function(mean_area_data) {
  ggplot(mean_area_data) +
    geom_brain(atlas = dk, 
               #position = position_brain(hemi ~ side),
               aes(fill = sub_mean)) +
    scale_fill_viridis_c(option = "plasma", direction = 1, limits = c(200, 9000), breaks = c(1000, 5000, 8000), labels = c("1000", "5000", "8000")) +
    theme_void()
}
...
# save(plots_brain_area_cv_orig, file = "plots_brain_area_cv_orig.RData")
# save(plots_brain_area_mean_orig, file = "plots_brain_area_mean_orig.RData") # save it for reuse

plots_brain_area_cv_qc <- plots_brain_cv
plots_brain_area_mean_qc <- plots_brain_mean
...
area_mean_sd <- data.frame(label=fs_data[["label"]]) %>%
  mutate(
...
process_subject <- function(subj) {
  ...
  colnames(lh_percent)[1:(ncol(lh_percent)-2)] <- fs_data_lh[["label"]]
  colnames(rh_percent)[1:(ncol(rh_percent)-2)] <- fs_data_lh[["label"]]
...
# save the plot
# plots_brain_area_per_change_orig <- plots_list_box
# save(plots_brain_area_per_change_orig, file = "plots_brain_area_per_change_orig.RData")

plots_brain_area_per_change_qc <- plots_list_box
save(plots_brain_area_per_change_qc, file = "plots_brain_area_per_change_qc.RData")
